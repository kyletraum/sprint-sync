...
- [x] T001 Register the `Microsoft.AzureActiveDirectory` resource provider on subscription `0970d4fa-d0f5-4d4d-9cd0-5e8ac9ae976b`. **Done 2026-08-25** — `az provider register` run, polled to `Registered`. Confirmed the provider still offers `ciamDirectories` **after** registration, not just before: R1 checked that while the provider was `NotRegistered`, and an offering can differ once a provider is actually registered. It does not here — the feature's central assumption holds.
- [x] T002 Confirm the azd environment exists and targets the right subscription and region. **Done 2026-08-25** — `sprintsync-dev` exists and is default: `AZURE_LOCATION=eastus2`, `AZURE_SUBSCRIPTION_ID=0970d4fa-d0f5-4d4d-9cd0-5e8ac9ae976b`, matching the handoff. `.azure/` confirmed untracked via `git check-ignore` — azd writes its own `.azure/.gitignore` containing `*`, so the environment and its secrets cannot be committed by accident. azd 1.31.2.
...
- [ ] T003 Create the Entra External ID (CIAM) tenant via the Azure portal (*Microsoft Entra External ID* → create external tenant). **Do not reuse "Default Directory"** — it is the MSA-backed directory owning the subscription; customers must not sign in to the directory that owns the billing account. Record the new tenant ID, which must differ from `az account show --query homeTenantId`. **Resource group decision (2026-08-25)**: created `rg-sprintsync-identity` specifically to hold it, NOT azd's `rg-sprintsync-dev`. An External ID tenant is an ARM resource in a resource group, so putting it in the azd-managed group would make `azd down` attempt to delete the identity tenant — with every app registration and test user in it — at teardown. The tenant must outlive the application's lifecycle. Chosen values: name `Sprint Sync`, domain `sprintsync`, residency `United States` (matches `eastus2`); all three are permanent.
...
- [x] T016 [US1] Author the Static Web App (Free tier) Bicep template in a **new `infra-web/` directory**, and invoke it from an `azure.yaml` postprovision hook the way `infra/budget.bicep` already is. **Do not put it in `infra/`** — that directory is generated by `azd infra gen` and the template would be erased, failing the CI infra-drift job. **Do not make it an azd service** — azd forbids pairing an Aspire service with a sibling, which is the documented reason the SPA was excluded originally (research R10). The declared-hook path is a first-class deploy path under constitution v1.1.0. The template MUST declare the **Free** SKU (FR-028). Decide and record whether `scripts/check-idle-cost.*` should extend its scan to `infra-web/` so that Free tier is machine-enforced rather than trusted — the guard currently scans `infra/` only, so this template would otherwise go unchecked.
- [x] T017 [P] [US1] Add a named-origin CORS policy to `src/SprintSync.Api/`, with the permitted origin read from configuration rather than compiled in. **The API has no CORS configuration today** (research R11). **No wildcard** — this is a credentialed, tenant-scoped API, and `AllowAnyOrigin` with credentials is both insecure and rejected by browsers. This is the one carve-out from "no changes to 001's behaviour": it is deployment-enabling configuration, and no endpoint, DTO or isolation behaviour changes.
- [x] T018 [P] [US1] Author `.github/workflows/deploy-web.yml` to build `web/sprint-sync-web` with `VITE_ENTRA_AUTHORITY`, `VITE_ENTRA_CLIENT_ID`, `VITE_API_SCOPE` and `VITE_API_BASE_URL` injected from repository secrets/variables, then deploy the built output to the Static Web App using its deployment token. Vite inlines these **at build time** (research R12) — they are not runtime configuration, so the SPA must be rebuilt whenever the tenant or API URL changes. Never commit the values (FR-029).

> **Web-hosting authoring record (2026-08-22)** — T016–T018 complete; T019–T021
> remain deploy-gated.
>
> - `infra-web/staticwebapp.bicep` — Free SKU, deployed by the `azure.yaml`
>   postprovision hook alongside `budget.bicep`. Kept **out** of `infra/` because
>   `azd infra gen` regenerates that tree and the CI infra-drift job diffs it, and
>   **not** made an azd service because azd forbids pairing an Aspire service with
>   a sibling — the documented reason the SPA was excluded originally.
> - `src/SprintSync.Api/Auth/SprintSyncCors.cs` + `Program.cs` — named-origin CORS
>   from `Cors:AllowedOrigins`, `UseCors` before authentication so unauthenticated
>   preflight `OPTIONS` is answered rather than rejected. No wildcard: this is a
>   credentialed API, where `AllowAnyOrigin` with credentials is both a hole and
>   browser-rejected. With no origins configured the policy permits nothing.
> - `.github/workflows/deploy-web.yml` — manual dispatch taking the API URL as an
>   input, building with the four `VITE_*` values from secrets, publishing with the
>   SWA deployment token. Fails fast with a named error when a secret is missing,
>   rather than shipping a bundle wired to an empty tenant.
>
> **T016's deferred decision is settled: YES, the guard scans `infra-web/`.**
> US4 exists because posture claims must be machine-checked, and a Free tier
> silently becoming Standard is exactly a standing-cost regression. Both twins now
> take a web-infra path and fail any `Microsoft.Web/staticSites` not on `Free`.
>
> **That immediately caught a real problem.** Defaulting the new path to the real
> `infra-web/` made every *fixture* run scan the real Static Web App template,
> which the fixtures' own `azure.yaml` files do not reference — so it classified as
> dead infrastructure and two previously-passing cases failed. Fixtures must be
> hermetic; the suite now passes the path explicitly. Worth noting the guard found
> the leak by failing, which is the behaviour it was rewritten to have.
>
> Suite now **18/18**, both twins agreeing on all six fixtures. Real repo green
> with `infra-web/` in scope.
...
**Goal**: CI is trustworthy evidence. **T055 done** — run 32828641212 on `304fb65`, four jobs green, job logs read to confirm non-vacuous work. **T054 done** — PR #6, run 32829224403 observed red with build passing and test failing, 1 of 58 failed, other three jobs green. **US5 is complete**: CI on this repository is now known to go red, not merely known to go green.

- [x] T054 [US5] Prove CI can **fail**: open a throwaway pull request containing one deliberately broken test, confirm the pipeline goes red, then close it without merging (research R8). A green pipeline nobody has seen go red is indistinguishable from one that always passes — this is the assumption under test, so it cannot be assumed. **Done 2026-08-25** — PR #6 (branched off `main`, one file, closed unmerged; branch deleted). Run 32829224403 went **red**, and the shape of the failure is what makes it evidence:
  - `Backend build & integration tests` failed while the **other three jobs passed** — so the red is attributable to one job, not a global outage that would have proven nothing.
  - Inside that job, **Build passed and Test failed**. This is the load-bearing detail: the job runs `dotnet test --no-build`, so build and test are separate steps. A compile error would have shown only that the build step runs; this shows the **test step observes results and propagates a non-zero exit**, which is the actual gating mechanism.
  - `Failed: 1, Passed: 57, Skipped: 0, Total: 58` — the suite was **not short-circuited**. Exactly the planted assertion failed and the other 57 tests still executed and passed, so a single failure does not mask the rest of the gate.
  - Failure text matches the plant: `Assert.Equal() Failure: Values differ` at `DeliberatelyBrokenTest.cs:line 24`, `Process completed with exit code 1`.
  - The test carried no fixture, collection or Docker dependency, so it could fail for exactly one reason — a red run could not be confused with Testcontainers failing to start.
