...
### Phase 9B: MIGRATED to feature 002 — Azure Deployment & Operability Baseline

**T055, T056, T057 and T058 no longer live here.** On 2026-08-22 they were moved
into `specs/002-azure-deploy-baseline/` (Azure Deployment & Operability
Baseline), which now owns all Azure infrastructure setup — including the Entra
External ID tenant and app registration, which nothing previously owned and
which hard-blocks `azd up`.

| Was | Now covered by 002 |
|---|---|
| T055 — real `azd up`, non-empty `AzureAd__*`, resolve the container-app module | User Story 1 (FR-005, FR-006) |
| T056 — cost guard must not be satisfied by undeployed infrastructure | User Story 4 (FR-009 … FR-013) |
| T057 — ACA probes + telemetry export | User Story 3 (FR-016 … FR-021) |
| T058 — live CI + deployed attack suite + Entra sign-in | User Stories 2 and 5 (FR-022 … FR-025) |

**T055's premise was disproved before the move.** Verified against azd 1.31.2:
`infra/api/api-containerapp.module.bicep` is **not** orphaned — it is azd's
per-service deploy-time module, regenerated byte-identical by `azd infra gen` and
deliberately excluded from `main.bicep` because it takes `api_containerimage`,
which cannot exist at provision time. `main.bicep`'s seven outputs are exactly
that module's remaining inputs. It must be neither wired in nor deleted. Carried
into 002's "Settled Decisions" so it is not re-investigated.

**T058's CI half was already complete and stays complete.** Workflow run
32537061874 on PR #3 was the first live execution of `.github/workflows/ci.yml`;
all four jobs (backend, web, idle-cost guard, infra drift) ran and passed, with
job logs read to confirm they do real work rather than passing vacuously. Only
the deploy-dependent half moved: a real `azd up`, the attack suite against a
deployed API, and an interactive Entra sign-in.

> **Principle IX evidence is DELEGATED, not discharged.** Feature 001's isolation
> guarantees remain proven only against Testcontainers. **001 MUST NOT be claimed
> production-done until feature 002's User Story 2 passes** and its result is
> recorded against the T048 validation record. Moving these tasks changed where
> the work is tracked — it did not lower the bar.

**Checkpoint**: Phase 9A is complete and merged without Azure. Feature 001 is
"verified in the build environment"; "verified in production" is feature 002's
User Story 2 to establish.
...
- **Handoff Follow-Ups (Phase 9)**: after Phase 8. **9A** (T050–T054) is complete. **9B** was migrated to feature 002 (`specs/002-azure-deploy-baseline/`) on 2026-08-22 — see the Phase 9B section. 001's Principle IX evidence gate is delegated there and remains open.
...
open. 9A needed nothing this repo did not already have, and is done.

9B turned out not to be a checklist but a feature: it required standing up an
Entra External ID tenant that nothing owned, and that hard-blocks `azd up` via
the preprovision guard. Rather than let deploy work accumulate as trailing tasks
on an application feature, it was migrated whole into feature 002. The "Not
validated" gap in T048's record stays open until 002 closes it.
